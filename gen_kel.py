...
# ---- BENCH tab (gated to Kel): shared full-estate bench map (see bench_render.py) ----
from bench_render import build_bench
BENCH_NAV,BENCH_PANEL=build_bench(REC,SHORT)
...
repl={
 ...
 "{{GEN_STAMP}}":GEN_STAMP,"{{FOOT_VIEW}}":"engagement","{{ROLE}}":ROLE,"{{SPEED_WIDGETS}}":SPEED_WIDGETS,"{{LW_LABEL}}":lw_label,"{{FOCUS_WEEK}}":focus_week,"{{NSTORES}}":str(len(stores)),"{{PILL}}":COACH+" · "+ROLE+" · all "+str(len(stores))+" stores","{{FOCUS_LI}}":focus_li,"{{QUEUE_BENCH}}":qbench_line(QBENCH.get("company") if QBENCH else None),
