...
open(os.path.join(OUT,"Bewiched_Star_Card_Leaderboard.html"),"w",encoding="utf-8").write(LEAD)
...
def toggle_bar():
    return ("<div class='tglwrap'><div class='tgl'><button data-w='qtd' class='on'>QTD</button><button data-w='ytd'>YTD</button></div>"
            "<div class='tglnote'>Switches the whole card &mdash; overall star, all four pillars &amp; each metric &mdash; between <b>QTD</b> (quarter to date) and <b>YTD</b> (year to date). Each tile still shows its <b>this-week</b> figure underneath.</div></div>")
def card_page(stores):
    hosts={w:"".join(card(c,w) for c in stores) for w in WINDOWS}
    return ("<!doctype html><html><head><meta charset='utf-8'><style>"+CARDCSS+"</style></head><body><div class='wrap'>"
            +toggle_bar()+"<div id='cardhost'>"+hosts['qtd']+"</div></div>"
            "<script>var HOST="+_json.dumps(hosts)+";"
            "document.querySelectorAll('.tgl button').forEach(function(b){b.onclick=function(){"
            "document.querySelectorAll('.tgl button').forEach(function(x){x.classList.remove('on')});"
            "b.classList.add('on');document.getElementById('cardhost').innerHTML=HOST[b.getAttribute('data-w')];};});"
            "</script></body></html>")
open(os.path.join(OUT,"Bewiched_Star_Card_Cards.html"),"w",encoding="utf-8").write(card_page(RANKED['qtd']))
open(os.path.join(OUT,"Bewiched_Star_Card_NorthamptonDT.html"),"w",encoding="utf-8").write(card_page(["Northampton Drive Thru"]))
AREA_PAGE=("<!doctype html><html><head><meta charset='utf-8'><style>"+CARDCSS+AREA_CSS+"</style></head><body><div class='wrap'>"
    +toggle_bar()+AREA_METHOD+"<div id='areahost'>"+AREA_VIEW['qtd']+"</div></div>"
    "<script>var AH="+_json.dumps(AREA_VIEW)+";document.querySelectorAll('.tgl button').forEach(function(b){b.onclick=function(){"
    "document.querySelectorAll('.tgl button').forEach(function(x){x.classList.remove('on')});b.classList.add('on');"
    "document.getElementById('areahost').innerHTML=AH[b.getAttribute('data-w')];};});</script></body></html>")
open(os.path.join(OUT,"Bewiched_Star_Card_Area.html"),"w",encoding="utf-8").write(AREA_PAGE)
print("AREA totals QTD:", {co:('%.2f'%AREA_OV['qtd'][co]) for co in AREAS}, " YTD:", {co:('%.2f'%AREA_OV['ytd'][co]) for co in AREAS})

for win in WINDOWS:
    print("==== %s ===="%WLAB[win].upper())
    print("RANK|STORE|OVERALL|Team|Ops|Cust|Profit|real")
    for c in RANKED[win]:
        S=D[c]['S'][win]; p=S['pillars']
        print("%2d|%-26s|%s|%s|%s|%s|%s|%d/8"%(RANK[win][c],c,fmt(S['overall']),fmt(p['Team']),fmt(p['Ops']),fmt(p['Customers']),fmt(p['Profit']),S['real_n']))
print("NDT QTD vs YTD:", {w:{'overall':fmt(D['Northampton Drive Thru']['S'][w]['overall']),
    'cust':fmt(D['Northampton Drive Thru']['S'][w]['pillars']['Customers']),
    'sales':D['Northampton Drive Thru']['S'][w]['sales'],'gc':D['Northampton Drive Thru']['S'][w]['gc']} for w in WINDOWS})
print("NDT Team(QTD) =", fmt(D['Northampton Drive Thru']['S']['qtd']['pillars']['Team']),
      "| RMS", D['Northampton Drive Thru'].get('rms'), "| RTW%", D['Northampton Drive Thru'].get('rtw'))
print("---- per-store RTW% (Team input) ----")
for c in CANON: print("%-26s RTW=%s  RMS=%s  Team=%s"%(c,D[c].get('rtw'),D[c].get('rms'),fmt(D[c]['S']['qtd']['pillars']['Team'])))
for _co in AREAS:
    _ac=area_card(_co,'qtd'); print("area_card %s: %d chars, overall %s"%(_co,len(_ac),fmt(AREA_OV['qtd'][_co])))
