...
def build():
    try:
        ...
        h = ("<!doctype html><html><head><meta charset='utf-8'><title>Franchise Fees Scale</title></head>"
             "<body><h2>Franchise Fees Scale</h2><p>Awaiting franchise_fees.json (%s).</p></body></html>" % _esc(e))
        open(os.path.join(HERE, "franchise-fees.html"), "w", encoding="utf-8").write(h)
    ...
    _cur = datetime.date.today().strftime("%Y-%m")
    _complete = [m for m in months if m < _cur]
    latest = (_complete[-1] if _complete else (months[-1] if months else None))
    ...
    def _store_row(st, m):
        ...
        brand = ("%.2f <span class='sub'>(%d)</span>" % (r["brand5"], r.get("brand_n", 0))) if r.get("brand5") is not None else "&mdash;"
        ...
        return ('<tr><td class="l">%s</td><td>%s</td><td>%s</td>'
                '<td class="cmb">%.2f</td><td class="cmb">%.1f</td><td>%s</td>'
                '<td>%s</td><td>%s</td><td class="tot">%s</td><td class="fee">%s %s</td></tr>'
                % (_esc(st), brand, rem, r["comb5"], r["comb100"], badge,
                   _pct(r.get("royalty")), _pct(r.get("marketing")), _pct(r.get("total_pct")), fee_gbp, turn))
    ...
    panels = ""
    for m in months:
        rows = "".join(_store_row(st, m) for st in stores) + "".join(_new_row(ns) for ns in new_stores)
        panels += (
    ...
    T.append("<title>Franchise Fees Scale &mdash; HoE / Ian's stores</title>")
    T.append("<style>" + CSS + "</style></head><body>")
    T.append('<div class="brandbar"><h1>&#127970; Franchise Fees Scale &mdash; HoE / Ian&rsquo;s stores</h1>'
             '<div class="sub">Royalty fees linked to brand execution &middot; keyed off the monthly Brand + Remote combined score '
             '(target 4.6/5 = 100/100) &middot; updated ' + _esc(updated) + '</div></div>')
    T.append('<div class="wrap">')
    T.append('<div class="bar"><label for="msel">Month</label><select id="msel">' + opts + '</select></div>')
    T.append('<div class="legend">Combined score = 50/50 blend of the <b>Brand audit</b> (/5) and <b>Remote assessment</b> '
             '(/100 &rarr; /5), on the /5 scale where <b>4.6 = the 100/100 target</b>. The royalty tier &amp; fee flex off that '
             'combined score; <b>&pound; Fee = Total Fee % &times; that month&rsquo;s turnover</b> (BigQuery). '
             'The view defaults to the last complete month; the current month fills as audits land. '
             'Months with no audit show &ldquo;no audit this month&rdquo;.</div>')
    T.append(panels)
    T.append('<div class="sech">Fee scale &mdash; royalty fees linked to brand execution (Proposed, NSO Month 2 onwards)</div>')
    T.append('<table class="scale"><thead><tr><th class="l">Combined score</th><th>Tier</th>'
             '<th>Royalty %</th><th>Marketing %</th><th>Total Fee %</th></tr></thead><tbody>' + srows + '</tbody></table>')
    T.append('<div class="foot">' + _esc(D.get("note", "")) +
             ' Boundary rule: a score at a band&rsquo;s lower bound sits in that band (4.6 = On Target, 4.9 = Excelling, 4.4 = Fail). '
             'Both percentages are of the store&rsquo;s turnover.</div>')
    T.append('</div><script>' + JS + '</script></body></html>')
    html = "\n".join(T)

    open(os.path.join(HERE, "franchise-fees.html"), "w", encoding="utf-8").write(html)
    print("[gen_franchise] wrote franchise-fees.html (%d months, %d stores + %d new; default %s)"
          % (len(months), len(stores), len(new_stores), latest))
